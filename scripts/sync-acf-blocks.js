...
function generateComponentTemplate(layout) {
    ...
    fields.forEach(field => {
        ...
        if (name.includes('title') || name.includes('heading')) {
            ...
            output += `          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3 mt-8">\n`;
            ...
            output += `                {/* Repeater item: ${name} */}\n`;
            output += `                <pre className="text-xs text-gray-400 overflow-auto">{JSON.stringify(item, null, 2)}</pre>\n`;
            output += `              </div>\n`;
            output += `            ))}\n`;
            output += `          </div>\n`;
            output += `        )}\n`;
...
function generateBlockRenderer(blocks) {
    ...
    blocks.sort((a, b) => a.layout.localeCompare(b.layout)).forEach(block => {
                ...
                if (field.type === 'flexible_content' && field.layouts) {
                    ...
                    layouts.forEach(layout => {
                        ...
                        if (existing) {
                            ...
                            const targetPath = path.join(NEXTJS_COMPONENTS_DIR, targetFolder, `${componentName}.jsx`);
