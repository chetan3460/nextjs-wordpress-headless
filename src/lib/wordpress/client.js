  ...
  try {
    ...
    if (!posts || posts.length === 0) {
      ...
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log(
            `[fetchPostBySlug] ${fallbackEndpoint} returned:`,
            posts?.length || 0,
            'results'
          );
        }
...
export async function fetchTeamMembersByCategory(categorySlug) {
  try {
    ...
    const members = await fetchREST(
      `/wp/v2/team_member?team_category=${categoryId}&per_page=100&_embed&orderby=menu_order&order=asc&acf_format=standard`
    );
